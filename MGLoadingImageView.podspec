#
# Be sure to run `pod lib lint MGLoadingImageView.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see http://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'MGLoadingImageView'
  s.version          = '0.1.0'
  s.summary          = 'A subclass on UIImageView that includes a built-in UIActivityIndicatorView'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
A subclass on UIImageView that includes a built-in UIActivityIndicatorView to optionally display a loading spinner in the foreground of the image view, to be used whilst loading an image.
                       DESC

  s.homepage         = 'https://github.com/mikegilroy/MGLoadingImageView'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'mikegilroy' => 'hello@mikegilroy.co' }
  s.source           = { :git => 'https://github.com/mikegilroy/MGLoadingImageView.git', :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/mike_gilroy'

  s.ios.deployment_target = '10.0'

  s.source_files = 'MGLoadingImageView/Classes/**/*'
  
  # s.resource_bundles = {
  #   'MGLoadingImageView' => ['MGLoadingImageView/Assets/*.png']
  # }

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit'
  # s.dependency 
end
